    ...
    <SiteShell>
      ...
      <section className="section alt strategy-summary">
        ...
        <div className="knowledge-callout">
          <div><b>新增交付：企业知识库 + 企业提示词体系</b><p>11表品牌事实知识库把27条信源、41条事实原子、30个问题和15条FAQ连成统一事实底座；提示词体系再把事实等级、边界规则和标准问答转成AI可执行的回答协议。</p></div>
          <div className="button-row">
      ...
      <section className="section strategy-kpi">
        ...
        <div className="download-panel"><div><b>完整执行文件</b><p>14页主方案、可编辑版、7工作表的90天执行工作簿、独立企业知识库，以及企业提示词体系。数据口径与本网站一致。更新：{updatedAt}</p></div><div className="button-row">{[...strategyDownloads, ...knowledgeDownloads].map((file) => <a className="button" href={file.href} download key={file.href}>{file.type}<ArrowIcon /></a>)}<Link className="button" href="/prompt-system">PROMPT<ArrowIcon /></Link></div></div>
